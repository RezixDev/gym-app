...
EXCLUDE_FILES=("package-lock.json" "pnpm-lock.yaml" "yarn.lock" "composer.lock" ".env.local")
...
eval "find $TARGET_DIR -type f \( $EXT_FILTER \) $EXCLUDE_STR" -print0 | while IFS= read -r -d '' file; do
    ...
    # NEW LOGIC: Check if file is in a /ui/ directory (shadcn components)
    if [[ "$file" == *"/ui/"* ]]; then
        echo "// [Code omitted: Shadcn UI Boilerplate]" >> "$OUTPUT_FILE"
    else
